...
def load_snapshot(
    path: Path, *, delimiter: str = ",", skip_header: int = 0
...
def auto_visualize_run(result: dict) -> None:
    ...
    run_label = str(result.get("run_label", run_dir.name))

    # Find snapshots
    snapshot_paths: List[Path] = []
    patterns = ["*snapshot*.csv", "snapshot*.csv", "*.csv", "*.dat", "*.txt"]
    for pat in patterns:
    ...
    if snapshot_paths:
        ...
        try:
            ...
            out_path = run_dir / f"{run_label}_snapshot_plot.png"
            plot_snapshot(
                x, u, title=f"Snapshot ({snap_path.name})", save_path=out_path
            )
        ...
        if len(snapshot_paths) >= 2:
            try:
                anim_path = run_dir / f"{run_label}_animation.mp4"
        ...
        try:
            ...
            if snapshot_paths:
                ...
                if x.shape == x_ref.shape:
                    ...
                    err = np.abs(u - u_ref)
                    err_plot_path = run_dir / f"{run_label}_error_plot.png"
                    plot_error(x, err, title="Pointwise error", save_path=err_plot_path)
                    print(
                        f"[viz] Error metrics L1={metrics.l1:.3e}, L2={metrics.l2:.3e}, Linf={metrics.linf:.3e}"
                    )
                    print(f"[viz] Saved error plot to {err_plot_path}")
                else:
                    print(
                        "[viz] Snapshot and reference shapes differ; skipping error plot."
                    )
        except Exception as exc:  # pylint: disable=broad-except
    ...
    if conv_path.exists():
        try:
            ...
            if data.ndim == 2 and data.shape[1] >= 2:
                dx = data[:, 0]
                err = data[:, 1]
                conv_plot_path = run_dir / f"{run_label}_convergence_plot.png"
                rate = plot_convergence(
                    dx, err, title="Convergence", save_path=conv_plot_path
                )
    ...
    energy_path = None
    for candidate in run_dir.glob("*energy*.csv"):
    ...
    if energy_path and energy_path.exists():
        try:
            ...
            if data.shape[1] >= 3:
                ...
                ax.grid(True)
                ax.legend()
                out_path = run_dir / f"{run_label}_energy_plot.png"
